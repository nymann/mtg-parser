...
#[derive(Debug, Serialize)]
struct NoPestFastpathDispatchReport {
    eligible_no_pest: bool,
...
fn run_grind(options: ConceptGrindOptions, sink: &mut dyn FlowSink) -> Result<()> {
    ...
    for iteration in 1..=options.max_iterations {
        ...
        let dispatch = dispatch_boundary_to_patch(&gap, &boundary_decision, true)?;
        write_json(
            iteration_dir.join("no_pest_fastpath_dispatch.json"),
            &dispatch,
        )?;
        if dispatch.eligible_no_pest {
            sink.emit(FlowEvent::Note {
                level: if dispatch.fastpath_result == "success" {
                    NoteLevel::Info
                } else {
                    NoteLevel::Warn
                },
                text: format!(
                    "no-PEST dispatch {} for {}{}",
                    dispatch.fastpath_result,
                    gap.concept,
                    dispatch
                        .fallback_reason
                        .as_ref()
        ...
        if dispatch.patch_agent_started {
            if dispatch.eligible_no_pest
                && !(dispatch.fastpath_attempted && dispatch.fastpath_result == "fallback")
            {
                metrics.step_finished(
                    sink,
                    &session_dir,
                    ...
                    4,
                    false,
                    Some("no_pest_fastpath_contract_failed".to_string()),
                )?;
                return Err(anyhow!(
                    "no_pest_fastpath_contract_failed: eligible no-PEST decision attempted to launch patch agent without fast-path fallback"
                ));
            }
            let patch_prompt =
...
fn dispatch_boundary_to_patch(
    gap: &ConceptGap,
    decision: &BoundaryDecision,
    write_files: bool,
) -> Result<NoPestFastpathDispatchReport> {
    let mut report = new_no_pest_fastpath_dispatch_report(gap, decision)?;

    if !report.eligible_no_pest {
    ...
    let pest_rules = vec![gap.target_rule.clone()];
    ...
    let Some(fixture_examples) = resolve_fastpath_fixture_examples(&examples, &pest_rules) else {
        ...
        report.fallback_reason = Some(format!(
            "one or more examples did not match target rule {}",
            gap.target_rule
        ));
        report.patch_agent_started = true;
        return Ok(report);
    ...
    if !counterexamples_reject_fastpath_rules(&counterexamples, &pest_rules) {
        report.fastpath_result = "fallback".to_string();
        report.fallback_reason = Some(format!(
            "one or more counterexamples matched target rule {}",
            gap.target_rule
        ));
        report.patch_agent_started = true;
        return Ok(report);
    ...
    if report.eligible_no_pest
        && report.patch_agent_started
        && !(report.fastpath_attempted && report.fastpath_result == "fallback")
    {
        bail!(
            "no_pest_fastpath_contract_failed: eligible no-PEST dispatch would launch patch agent without an attempted fallback"
        );
    }
...
fn new_no_pest_fastpath_dispatch_report(
    gap: &ConceptGap,
    decision: &BoundaryDecision,
) -> Result<NoPestFastpathDispatchReport> {
    let normalized_owner = normalized_boundary_owner(&decision.owner);
    let normalized_pest_patch_intent = normalize_pest_patch_intent(&decision.pest_patch_intent);
    let target_rule_exists = grammar_query_engine::parse_grammar_file(grammar_pest_path())?
        .iter()
        .any(|rule| rule.name == gap.target_rule);
    let scaffold_inputs_present = !boundary_axes(&decision.axes).is_empty()
        && !boundary_text_items(&decision.examples_to_accept).is_empty()
        && !boundary_text_items(&decision.counterexamples_to_reject).is_empty();
    let eligible_no_pest = normalized_owner == "new"
        && normalized_pest_patch_intent == "none"
        && target_rule_exists
        && scaffold_inputs_present;
    let fallback_reason = if eligible_no_pest {
        None
    } else {
        Some(no_pest_fastpath_ineligible_reason(
            &normalized_owner,
    ...
    let fastpath_result = if eligible_no_pest {
        "pending"
    } else {
        "not_eligible"
    };
    Ok(NoPestFastpathDispatchReport {
        eligible_no_pest,
        normalized_owner,
        normalized_pest_patch_intent,
        target_rule_exists,
        ...
        fastpath_result: fastpath_result.to_string(),
        fallback_reason,
...
fn normalized_boundary_owner(owner: &BoundaryOwner) -> String {
    match owner {
        BoundaryOwner::Existing(_) => "existing",
        BoundaryOwner::New(_) => "new",
        BoundaryOwner::Blocked(_) => "blocked",
    }
    .to_string()
}

fn normalize_pest_patch_intent(intent: &str) -> String {
    let trimmed = intent.trim().to_ascii_lowercase();
    if trimmed == "none"
...
fn no_pest_fastpath_ineligible_reason(
    normalized_owner: &str,
    normalized_pest_patch_intent: &str,
    target_rule_exists: bool,
    scaffold_inputs_present: bool,
) -> String {
    if normalized_owner != "new" {
        return format!("OWNER normalized to {normalized_owner}, not new");
    }
    if normalized_pest_patch_intent != "none" {
        return format!("PEST_PATCH_INTENT normalized to {normalized_pest_patch_intent}, not none");
    }
    if !target_rule_exists {
        return "target rule does not exist".to_string();
    }
    if !scaffold_inputs_present {
        return "concept/fixture scaffold inputs are incomplete".to_string();
    }
    "not eligible for no-PEST fast path".to_string()
}
...
fn boundary_axes(text: &str) -> Vec<ConceptFastpathAxis> {
    boundary_axis_items(text)
        ...
        .filter_map(|item| {
            ...
            Some(ConceptFastpathAxis {
                name,
                values: boundary_axis_values(values),
            })
        })
        .collect()
}

fn boundary_axis_items(text: &str) -> Vec<String> {
    text.lines()
...
fn boundary_text_items(text: &str) -> Vec<String> {
    let quoted = quoted_boundary_items(text);
    if !quoted.is_empty() {
        return quoted;
    }
    text.lines()
...
fn quoted_boundary_items(text: &str) -> Vec<String> {
    ...
    while let Some(ch) = chars.next() {
        ...
        let item = clean_boundary_item(&item);
        if !item.is_empty() {
            items.push(item);
        }
    }
...
mod tests {
    ...
    #[test]
    fn batch_12_eligible_no_pest_decisions_enter_dispatch_before_patch_agent() {
        let cases = [
            (
                "optional_player_shuffle_instruction",
                "optional player shuffle instruction",
                "you_may_have_that_player_shuffle",
                "CONCEPT_BOUNDARY_DECISION:\n\
                 OWNER: new:optional_player_shuffle_instruction\n\
                 AXES: action=shuffle_keyword_action; modality=optional_may; instructor=you; actor_ref=that_player; object=implicit_library_or_referenced_library_context\n\
                 EXAMPLES_TO_ACCEPT: \"You may have that player shuffle.\"\n\
                 COUNTEREXAMPLES_TO_REJECT: \"Shuffle.\"; \"Each player shuffles their hand and graveyard into their library.\"; \"You may have that player draw a card.\"; \"You may shuffle.\"\n\
                 PEST_PATCH_INTENT: none for grammar shape; create/map a concept fixture for the existing complete sentence rule, preserving `shuffle_action` as lower-level imperative plumbing.\n\
                 WHY_NOT_CARD_PASS: grammar fixture maturity only\n",
            ),
            (
                "tap_source_action",
                ...
                "tap_source_action",
                "CONCEPT_BOUNDARY_DECISION:\n\
                 OWNER: new:tap_source_action\n\
                 AXES: source_object = [\"CARDNAME\", \"this_source_object_kind\", \"that_source\"]; context = [\"imperative_action_sequence\", \"unless_you_pay_mana_do_actions\"]\n\
                 EXAMPLES_TO_ACCEPT: \"tap CARDNAME\"; \"tap this creature\"; \"tap that source\"\n\
                 COUNTEREXAMPLES_TO_REJECT: \"Tap all lands target player controls and that player loses all unspent mana.\"; \"tap all lands target player controls.\"; \"untap CARDNAME.\"; \"CARDNAME enters tapped.\"; \"tap target creature.\"\n\
                 PEST_PATCH_INTENT: none\n\
                 WHY_NOT_CARD_PASS: grammar fixture maturity only\n",
            ),
            (
                "tap_target_permanent_choice",
                "tap target permanent choice",
                "tap_target_permanent_choice",
                "CONCEPT_BOUNDARY_DECISION:\n\
                 OWNER: new:tap_target_permanent_choice\n\
                 AXES: preserve `tap_source_action.source_object` and `tap_source_action.context`; new concept axes: `permission = [required, you_may]`, `tap_mode = [tap, tap_or_untap]`, `target_selector = [target_permanent, target_permanent_type_choice, target_creature_type]`, `context = [sentence_statement, activated_effect]`\n\
                 EXAMPLES_TO_ACCEPT: `tap target creature.`, `you may tap target permanent.`, `tap or untap target artifact.`\n\
                 COUNTEREXAMPLES_TO_REJECT: `tap CARDNAME`, `tap this creature`, `untap target creature.`, `tap all lands target player controls.`, `CARDNAME enters tapped.`\n\
                 PEST_PATCH_INTENT: none; do not widen `tap_source_action`. Map `tap_target_permanent_choice` under a separate targeted tap/optional-untap action concept.\n\
                 WHY_NOT_CARD_PASS: grammar fixture maturity only\n",
            ),
            (
                "tap_enchanted_object_action",
                "tap enchanted object action",
                "tap_enchanted_object",
                "CONCEPT_BOUNDARY_DECISION:\n\
                 OWNER: new:tap_enchanted_object_action\n\
                 AXES: action=tap; enchanted_object=permanent_type|creature_type; context=trigger_effect\n\
                 EXAMPLES_TO_ACCEPT: \"tap enchanted creature.\"; \"tap enchanted land.\"; \"tap enchanted artifact.\"\n\
                 COUNTEREXAMPLES_TO_REJECT: \"tap CARDNAME\"; \"tap this creature\"; \"tap that source\"; \"tap target creature.\"; \"untap enchanted creature.\"\n\
                 PEST_PATCH_INTENT: none; keep `tap_enchanted_object` separate and map it as a new Aura-enchanted-object tap concept rather than widening `tap_source_action`\n\
                 WHY_NOT_CARD_PASS: grammar fixture maturity only\n",
            ),
            (
                "mana_ability_activation_instruction",
                "mana ability activation instruction",
                "target_player_activates_mana_ability_of_each_permanent_they_control",
                "CONCEPT_BOUNDARY_DECISION:\n\
                 OWNER: new:mana_ability_activation_instruction\n\
                 AXES: actor=target_player; action=activate; ability_kind=mana_ability; ability_quantity=article_singular; source_scope=each_controlled_permanent; source_permanent_type=artifact|creature|enchantment|land|planeswalker; context=standalone_sentence\n\
                 EXAMPLES_TO_ACCEPT: \"Target player activates a mana ability of each land they control.\"; \"Target player activates a mana ability of each artifact they control.\"; \"Target player activates a mana ability of each creature they control.\"\n\
                 COUNTEREXAMPLES_TO_REJECT: \"While doing so, the player can activate mana abilities only if they're from lands that player controls and only if mana they produce is spent to activate other mana abilities of lands the player controls and/or to play that card.\"; \"Until end of turn, any time you could activate a mana ability, you may pay 1 life.\"; \"Whenever a player taps a land for mana, that player adds one mana of any type that land produced.\"; \"Tap all lands target player controls and that player loses all unspent mana.\"; \"Activated abilities of white enchantments cost {3} more to activate.\"\n\
                 PEST_PATCH_INTENT: none; map the existing `target_player_activates_mana_ability_of_each_permanent_they_control` rule to this new concept and cover it with grammar fixtures.\n\
                 WHY_NOT_CARD_PASS: grammar fixture maturity only\n",
            ),
            (
                "mana_ability_activation_restriction",
                "mana ability activation restriction",
                "mana_ability_activation_limit",
                "CONCEPT_BOUNDARY_DECISION:\n\
                 OWNER: new:mana_ability_activation_restriction\n\
                 AXES: actor=controlled_player; action=activate; ability_kind=mana_ability; permission_polarity=can_only_if; context=while_doing_so; source_scope=lands_controlled_by_that_player; spending_limit=mana_spent_to_activate_other_mana_abilities_or_play_referenced_card\n\
                 EXAMPLES_TO_ACCEPT: \"While doing so, the player can activate mana abilities only if they're from lands that player controls and only if mana they produce is spent to activate other mana abilities of lands the player controls and/or to play that card.\"\n\
                 COUNTEREXAMPLES_TO_REJECT: \"Target player activates a mana ability of each land they control.\"; \"Until end of turn, any time you could activate a mana ability, you may pay 1 life.\"; \"Activated abilities of white enchantments cost {3} more to activate.\"\n\
                 PEST_PATCH_INTENT: none; this is a concept-map boundary decision. Do not widen `mana_ability_activation_instruction`, because the target rule is conditional permission/restriction text, not a standalone instruction to activate mana abilities.\n\
                 WHY_NOT_CARD_PASS: grammar fixture maturity only\n",
            ),
        ];

        for (concept, query, target_rule, response) in cases {
            ...
            let gap = ConceptGap {
                ...
                target_line: 1,
                suggested_existing_owner: false,
                reason: "batch 12 no-PEST dispatch regression".to_string(),
            };
            let pending =
                new_no_pest_fastpath_dispatch_report(&gap, &decision).expect("build report");
            assert!(pending.eligible_no_pest, "{concept} should be eligible");
            assert!(!pending.fastpath_attempted);
            assert!(!pending.patch_agent_started);

            let dispatched =
                dispatch_boundary_to_patch(&gap, &decision, false).expect("dispatch fastpath");
            assert!(
                dispatched.eligible_no_pest,
                "{concept} should stay eligible"
            );
            assert_eq!(dispatched.normalized_owner, "new");
            assert_eq!(dispatched.normalized_pest_patch_intent, "none");
            assert!(dispatched.target_rule_exists);
            assert!(dispatched.fastpath_attempted);
            if dispatched.patch_agent_started {
                assert_eq!(dispatched.fastpath_result, "fallback");
                assert!(
                    dispatched.fallback_reason.is_some(),
                    "{concept} fallback should explain why"
                );
            } else {
                assert_eq!(dispatched.fastpath_result, "success");
            }
        }
    }
