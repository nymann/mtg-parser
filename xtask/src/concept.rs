...
#[derive(Debug, Serialize)]
struct ConceptCandidateSelectionReport {
    excluded_count: usize,
    excluded_rules: Vec<String>,
    structural_exclusions: Vec<StructuralExclusion>,
    remaining_candidate_count: usize,
    selected_post_filter_candidate: String,
}

struct GrammarTopology {
...
fn concept_candidate_selection_report(
    report: &ExistingGrammarMapReport,
    excluded_rules: &BTreeSet<String>,
    structural_exclusions: Vec<StructuralExclusion>,
    gap: &ConceptGap,
) -> ConceptCandidateSelectionReport {
    ...
    ConceptCandidateSelectionReport {
        ...
        excluded_rules: excluded_rules.iter().cloned().collect(),
        structural_exclusions,
        remaining_candidate_count,
        selected_post_filter_candidate: gap.target_rule.clone(),
    }
}

fn classify_structural_non_concept_candidates(
    report: &ExistingGrammarMapReport,
) -> Result<Vec<StructuralExclusion>> {
    let rules = grammar_query_engine::parse_grammar_file(grammar_pest_path())?;
    Ok(classify_structural_non_concept_candidates_from_rules(
        report, &rules,
    ))
}

fn classify_structural_non_concept_candidates_from_rules(
    report: &ExistingGrammarMapReport,
    rules: &[grammar_query_engine::GrammarRuleDefinition],
) -> Vec<StructuralExclusion> {
    let rule_by_name: BTreeMap<&str, &grammar_query_engine::GrammarRuleDefinition> = rules
        .iter()
        .map(|rule| (rule.name.as_str(), rule))
        .collect();
    let topology = GrammarTopology::new(rules);
    let unmapped_names: BTreeSet<&str> = report
        .unmapped_rules
        .iter()
    ...
    for candidate in &report.unmapped_rules {
        ...
        if let Some(exclusion) = classify_wrapper_or_alternation_non_concept(
            report,
            rules,
            &topology,
            &rule_by_name,
        ...
        if let Some(exclusion) =
            classify_value_definition_payload_non_concept(report, &topology, &unmapped_names, rule)
        {
            exclusions.push(exclusion);
            continue;
        }
        if let Some(exclusion) = classify_lexical_payload_non_concept(report, &topology, rule) {
            exclusions.push(exclusion);
        }
    }
...
impl GrammarTopology {
    fn new(rules: &[grammar_query_engine::GrammarRuleDefinition]) -> Self {
        ...
        for rule in rules {
            ...
            direct.insert(rule.name.clone(), dependencies);
        }
        Self { direct, reverse }
    }

    fn direct_dependencies(&self, rule_name: &str) -> Vec<String> {
        self.direct.get(rule_name).cloned().unwrap_or_default()
    }

    fn reverse_dependencies(&self, rule_name: &str) -> Vec<String> {
        self.reverse.get(rule_name).cloned().unwrap_or_default()
    }
}

fn classify_wrapper_or_alternation_non_concept(
    report: &ExistingGrammarMapReport,
    rules: &[grammar_query_engine::GrammarRuleDefinition],
    topology: &GrammarTopology,
    rule_by_name: &BTreeMap<&str, &grammar_query_engine::GrammarRuleDefinition>,
    rule: &grammar_query_engine::GrammarRuleDefinition,
) -> Option<StructuralExclusion> {
    let dependencies = topology.direct_dependencies(&rule.name);
    if !is_structural_wrapper_or_alternation_rhs(&rule.rhs, &dependencies) {
        return None;
    }

    let expansion_tree = expand_rule_leaf_set(rules, rule_by_name, &rule.name, 3, None);
    let leaf_rules: BTreeSet<String> = expansion_tree
        .normalized_leaf_rules
        .iter()
        .cloned()
        .collect();
    let leaf_owners = leaf_owning_concepts(report, &leaf_rules);
    let owner_count = leaf_owners
        .values()
        .filter(|owners| !owners.is_empty())
        .flatten()
        .collect::<BTreeSet<_>>()
        .len();
    let owned_leaf_count = leaf_owners
        .values()
        .filter(|owners| !owners.is_empty())
        .count();
    if owner_count < 2 || owned_leaf_count < 2 {
        return None;
    }

    Some(StructuralExclusion {
        target_rule: rule.name.clone(),
        structural_exclusion_reason:
            "wrapper/alternation rule whose meaningful children are already mapped to separate concepts"
                .to_string(),
        matched_feature: format!(
            "pure_wrapper_or_alternation; owned_leaf_count={owned_leaf_count}; owner_count={owner_count}"
        ),
        evidence_rule_or_parent: leaf_owners
            .into_iter()
            .filter(|(_, owners)| !owners.is_empty())
            .map(|(leaf, owners)| format!("{leaf}->{}", owners.join("|")))
            .collect::<Vec<_>>()
            .join(", "),
    })
}

fn is_structural_wrapper_or_alternation_rhs(rhs: &str, dependencies: &[String]) -> bool {
    if !is_pure_wrapper_or_alternation_rhs(rhs, dependencies)
        && rhs_quoted_literals_contain_ascii_letters(rhs)
    {
        return false;
    }
    ...
    pest_rhs_identifier_tokens(rhs).into_iter().all(|token| {
        is_pest_wrapper_builtin_identifier(&token) || dependency_names.contains(token.as_str())
    })
}

fn rhs_quoted_literals_contain_ascii_letters(rhs: &str) -> bool {
    ...
    while let Some(ch) = chars.next() {
        ...
        for literal_ch in chars.by_ref() {
            ...
            if literal_ch.is_ascii_alphabetic() {
                return true;
            }
        }
    }
...
fn classify_value_definition_payload_non_concept(
    report: &ExistingGrammarMapReport,
    topology: &GrammarTopology,
    unmapped_names: &BTreeSet<&str>,
    rule: &grammar_query_engine::GrammarRuleDefinition,
) -> Option<StructuralExclusion> {
    let dependencies = topology.direct_dependencies(&rule.name);
    let is_value_definition = dependencies
        .iter()
        .any(|dependency| dependency == "value_expression")
        || rule.name.ends_with("_variable_definition");
    if !is_value_definition {
        return None;
    }

    let parent = semantic_parent_for_payload_helper(report, topology, unmapped_names, &rule.name)?;
    Some(StructuralExclusion {
        target_rule: rule.name.clone(),
        structural_exclusion_reason:
            "where-clause/value-definition payload helper whose parent concept owns the semantic surface"
                .to_string(),
        matched_feature: format!(
            "value_definition_payload; dependencies={}",
            dependencies.join("|")
        ),
...
fn classify_lexical_payload_non_concept(
    report: &ExistingGrammarMapReport,
    topology: &GrammarTopology,
    rule: &grammar_query_engine::GrammarRuleDefinition,
) -> Option<StructuralExclusion> {
    ...
    let atomic_or_silent = rule.rhs.trim_start().starts_with("@{")
        || rule.rhs.trim_start().starts_with("_{")
        || rule.rhs.trim_start().starts_with("${");
    if !has_literal || !atomic_or_silent {
        return None;
    }
    if rule_has_direct_unparse_writer(&rule.name) {
        return None;
    }
    let parent = semantic_parent_for_lexical_helper(report, topology, &rule.name)?;
    Some(StructuralExclusion {
        target_rule: rule.name.clone(),
        structural_exclusion_reason:
            "lexical/token rule with no semantic AST ownership outside a parent concept surface"
                .to_string(),
        matched_feature: format!(
            "lexical_token; atomic_or_silent={atomic_or_silent}; direct_ast_surface={}; dependencies={}",
            rule_has_direct_semantic_ast_surface(&rule.name),
            dependencies.join("|")
        ),
        evidence_rule_or_parent: parent,
    })
...
fn semantic_parent_for_payload_helper(
    report: &ExistingGrammarMapReport,
    topology: &GrammarTopology,
    unmapped_names: &BTreeSet<&str>,
    rule_name: &str,
) -> Option<String> {
    for parent in topology.reverse_dependencies(rule_name) {
        if !parent.contains("where") && !parent.ends_with("_variable_definition") {
            continue;
        }
        if let Some(mapped_parent) = nearest_mapped_reverse_dependency(report, topology, &parent, 2)
        {
            return Some(format!("{parent} via {mapped_parent}"));
        }
    }
    nearest_mapped_reverse_dependency(report, topology, rule_name, 2)
        .map(|parent| format!("{rule_name} via {parent}"))
        .or_else(|| {
            let shared_parent = topology
                .reverse_dependencies(rule_name)
                .into_iter()
                .find(|parent| !unmapped_names.contains(parent.as_str()))?;
            Some(format!("{rule_name} via {shared_parent}"))
        })
}

fn semantic_parent_for_lexical_helper(
    report: &ExistingGrammarMapReport,
    topology: &GrammarTopology,
    rule_name: &str,
) -> Option<String> {
    nearest_mapped_reverse_dependency(report, topology, rule_name, 2)
        ...
        .or_else(|| {
            let parent = topology
                ...
                .into_iter()
                .next()?;
            Some(format!("{rule_name} via {parent}"))
        })
}

fn nearest_mapped_reverse_dependency(
    report: &ExistingGrammarMapReport,
    topology: &GrammarTopology,
    rule_name: &str,
    max_depth: u8,
) -> Option<String> {
    let owned_rules: BTreeMap<&str, Vec<&str>> = report
        .concepts
        .iter()
        .flat_map(|concept| {
            concept
                .owned_rules
                .iter()
                .map(move |owned| (owned.name.as_str(), concept.concept.as_str()))
        })
        .fold(BTreeMap::new(), |mut acc, (rule, concept)| {
            acc.entry(rule).or_default().push(concept);
            acc
        });
    let mut seen = BTreeSet::new();
    let mut frontier = vec![(rule_name.to_string(), 0u8)];
    while let Some((current, depth)) = frontier.pop() {
        if depth > max_depth || !seen.insert(current.clone()) {
            continue;
        }
        if current != rule_name {
            if let Some(owners) = owned_rules.get(current.as_str()) {
                return Some(format!("{current}->{}", owners.join("|")));
            }
        }
        for parent in topology.reverse_dependencies(&current) {
...
fn rule_has_direct_semantic_ast_surface(rule_name: &str) -> bool {
    let (parse, ast, unparse) = grammar_semantic_source_texts();
    let parser_fn = format!("fn {rule_name}_from_pair");
    let writer_fn = format!("fn write_{rule_name}");
    let enum_name = rule_name
        .split('_')
        ...
        .collect::<String>();
    parse.contains(&parser_fn)
        && (ast.contains(&format!("enum {enum_name}"))
            || ast.contains(&format!("struct {enum_name}"))
            || unparse.contains(&writer_fn))
}

fn rule_has_direct_unparse_writer(rule_name: &str) -> bool {
    let (_, _, unparse) = grammar_semantic_source_texts();
    unparse.contains(&format!("fn write_{rule_name}("))
}

fn grammar_semantic_source_texts() -> &'static (String, String, String) {
    static SOURCES: OnceLock<(String, String, String)> = OnceLock::new();
    SOURCES.get_or_init(|| {
        (
            fs::read_to_string(repo_root().join("crates/mtg-grammar/src/parse.rs"))
                .unwrap_or_default(),
            fs::read_to_string(repo_root().join("crates/mtg-grammar/src/ast.rs"))
                .unwrap_or_default(),
            fs::read_to_string(repo_root().join("crates/mtg-grammar/src/unparse.rs"))
                .unwrap_or_default(),
        )
    })
}
...
fn expand_rule_leaf_set(
    ...
    let pure_wrapper_or_alternation =
        rule.is_some_and(|rule| is_structural_wrapper_or_alternation_rhs(&rule.rhs, &dependencies));
    let should_expand =
...
fn run_grind(options: ConceptGrindOptions, sink: &mut dyn FlowSink) -> Result<()> {
    ...
    for iteration in 1..=options.max_iterations {
        ...
        let structural_exclusions = classify_structural_non_concept_candidates(&before)?;
        excluded_rules.extend(
            structural_exclusions
                .iter()
                .map(|exclusion| exclusion.target_rule.clone()),
        );
        let gap = select_concept_gap_excluding(&before, &options, &excluded_rules)?;
        let selection_report = concept_candidate_selection_report(
            &before,
            &excluded_rules,
            structural_exclusions,
            &gap,
        ...
        write_json(
            iteration_dir.join("concept_candidate_selection.json"),
            &selection_report,
        )?;
...
"#,
    ...
    #[test]
    fn selector_replay_excludes_batch7_structural_non_concepts_before_boundary_prompt() {
        let report = run_map_existing(MapExistingOptions {
            json: true,
            expand_deps: true,
        })
        .expect("map existing grammar");
        let rules =
            grammar_query_engine::parse_grammar_file(grammar_pest_path()).expect("grammar parses");
        let exclusions = classify_structural_non_concept_candidates_from_rules(&report, &rules);
        let by_rule: BTreeMap<_, _> = exclusions
            .iter()
            .map(|exclusion| (exclusion.target_rule.as_str(), exclusion))
            .collect();
        ...
        for rule in [
            ...
            let exclusion = by_rule
                .get(rule)
                .unwrap_or_else(|| panic!("{rule} should be structurally excluded"));
            assert!(
                !exclusion.structural_exclusion_reason.is_empty(),
                "{rule} should log structural_exclusion_reason"
            );
            assert!(
                !exclusion.matched_feature.is_empty(),
                "{rule} should log matched_feature"
            );
            assert!(
                !exclusion.evidence_rule_or_parent.is_empty(),
                "{rule} should log evidence_rule_or_parent"
            );
        }

        assert!(by_rule["spell_type"]
            .matched_feature
            .contains("lexical_token"));
        assert!(by_rule["upper_alpha"]
            .matched_feature
            .contains("lexical_token"));
        assert!(by_rule["variable_definition"]
            .matched_feature
            .contains("value_definition_payload"));
        assert!(by_rule["colored_target_effect"]
            .matched_feature
            .contains("pure_wrapper_or_alternation"));
        assert!(by_rule["colored_target_action"]
            .matched_feature
            .contains("pure_wrapper_or_alternation"));
        assert!(by_rule["colored_target_action"]
            .evidence_rule_or_parent
            .contains("counter_target_spell"));
        assert!(by_rule["colored_target_action"]
            .evidence_rule_or_parent
            .contains("destroy"));

        let excluded_rules: BTreeSet<String> = exclusions
            .iter()
            .map(|exclusion| exclusion.target_rule.clone())
            .collect();
        let options = ConceptGrindOptions {
        ...
        let gap = select_concept_gap_excluding(&report, &options, &excluded_rules)
            .expect("selector should choose a post-filter candidate");
        assert!(
            ![
                "spell_type",
                "variable_definition",
            ...
            .contains(&gap.target_rule.as_str()),
            "replay-excluded candidates must not reach boundary prompt construction"
        );
        let selection_report =
            concept_candidate_selection_report(&report, &excluded_rules, exclusions, &gap);
        assert_eq!(
            selection_report.selected_post_filter_candidate,
            gap.target_rule
        );
        assert!(selection_report.excluded_count >= 5);
        assert!(selection_report.remaining_candidate_count > 0);
    }
